...
def _ingest_paper_internal(input_str: str) -> ParsedPaper:
...
def ingest_paper(input_str: str) -> ParsedPaper:
    try:
        parsed = _ingest_paper_internal(input_str)
        text_content = "".join(parsed.pages).strip()
        if len(text_content) < 500:
            raise ValueError("Document contains very little readable text (possibly a scanned PDF without OCR).")
        return parsed
    except Exception as e:
        print(f"\n[WARNING] Failed to ingest or read the paper from '{input_str}'.")
        print(f"Reason: {str(e)}")
        while True:
            choice = input("Would you like to:\n[1] Provide a local path to a readable PDF file\n[2] Abort\nEnter choice (1/2): ").strip()
            if choice == "1":
                new_path = input("Enter the full path to the PDF file: ").strip()
                if Path(new_path).exists() and Path(new_path).suffix.lower() == ".pdf":
                    try:
                        new_paper = parse_pdf(new_path, {"input_source": new_path, "title": Path(new_path).stem, "authors": []})
                        text_content = "".join(new_paper.pages).strip()
                        if len(text_content) < 500:
                            print("The provided PDF also seems unreadable (too short or no text). Please try another.")
                            continue
                        return new_paper
                    except Exception as parse_err:
                        print(f"Failed to parse new PDF: {parse_err}")
                else:
                    print("Invalid path or not a PDF.")
            elif choice == "2":
                raise ValueError("Paper ingestion aborted by user.")
            else:
